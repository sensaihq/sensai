...
): Options | undefined => { // TODO replace output type
...
const getSourceMapsOptions = (): Pick<Options, 'sourceMaps' | 'inlineSourcesContent'> => {
  return {
...
const getSyntax = (extension: JSExtensionE): FileSyntaxE => {
  return extension === JSExtensionE.TYPESCRIPT ? FileSyntaxE.TYPESCRIPT : FileSyntaxE.JAVASCRIPT
}
