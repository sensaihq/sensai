...
import { HTTP_ANY, HTTP_DEFAULT_METHOD, HTTP_GET, HTTP_HEAD, HTTP_STATUS, MIME_TYPE, VERSION_DEFAULT } from "@/src/constants"
...
export default (router: Router) => {
  ...
  return async (request: IncomingMessage, response: ServerResponse) => {
    ...
    if (resource) {
      ...
      const endpoint = resource[isHead ? HTTP_GET : method] || resource[HTTP_ANY]
      if (endpoint) {
        ...
        if (route) {
          ...
          try {
            const output = await context.run({}, async () => {
              return await [...middlewares, routePath].reduce((prev, next) => {
        ...
        write(response, HTTP_STATUS.NOT_ALLOWED, { 'Allow': Object.keys(resource) })
      }
    } else {
...
const write = (response: ServerResponse, code: number, headers: any = {}, payload?: string) => {
